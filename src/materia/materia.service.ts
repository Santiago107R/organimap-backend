...
export class MateriaService {
  ...
  async findAll(paginationDto: PaginationDto) {
      ...
      const [cursos, total] = await this.materiaRepository.findAndCount({
        take: limit,
        skip: offset,
        where: {
    ...
    async findOne(id: string) {
      const curso = await this.materiaRepository.findOneBy({ id });
  
      if (!curso) throw new NotFoundException(`Materia with id ${id} not found`);
